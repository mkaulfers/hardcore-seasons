...
public class RewardManager {
    ...
    public void saveRewards(List<Participant> winners) {
        ...
        List<EndChest> endChests = plugin.db.endChests.getEndChests(currentSeasonNum);
        List<ParticipantInventory> inventories = plugin.db.inventories.getInventories(currentSeasonNum);
        List<TrackedContainer> trackedContainers = plugin.db.containers.getTrackedContainers(currentSeasonNum);
        ...
        plugin.db.endChests.deleteEndChests(currentSeasonNum);
        plugin.db.inventories.deleteInventories(currentSeasonNum);
        plugin.db.containers.deleteTrackedContainers(currentSeasonNum);
    ...
    private void saveWinnerRewardsToDatabase(Map<Participant, List<ItemStack>> winnerRewards) {
        ...
        for (Map.Entry<Participant, List<ItemStack>> entry : winnerRewards.entrySet()) {
            ...
            seasonReward.setSeasonId(plugin.activeSeason.getSeasonId());
